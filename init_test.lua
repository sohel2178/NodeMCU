...
function set_state(data)
     ...
     gpio.write(pin_five,arr[1])
     gpio.write(pin_six,arr[2])
     gpio.write(pin_seven,arr[3])
     gpio.write(pin_eight,arr[4])
end
...
function read_from_pir(state)
       
    if pir_timer == nil then
        pir_timer = tmr.create()
    end

    if state then
        my_str = ""
        pir_timer:register(3000, tmr.ALARM_AUTO, function() 
            my_str = my_str .. gpio.read(pin_three)
            print(my_str)
   
            if string.len(my_str)>=100 then
               my_str = string.sub(my_str,-100)
            end
...
function process_response_from_socket(sck,data)
    ...
    elseif command == "ACTIVATE_PIR" then
        -- Activate PIR 
    end
end
...
function get_pin_state()
    return gpio.read(pin_five) .. gpio.read(pin_six) .. gpio.read(pin_seven) .. gpio.read(pin_eight)
end
...
function setup_gpio_pins()
    -- Permanent wifi indicator
    pin_one=1 
    -- Temporary wifi indicator
    pin_two=2
    -- PIR PIN
    pin_three=3
    ...
    gpio.mode(pin_five, gpio.OUTPUT)
    gpio.mode(pin_six, gpio.OUTPUT)
